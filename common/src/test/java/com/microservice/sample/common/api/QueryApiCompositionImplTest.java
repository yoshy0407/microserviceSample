...
class QueryApiCompositionImplTest {
	...
	void test() {
		...
		QueryApiCompositionImpl<BaseEntity, ResultEntity> apiComposition = 
				new QueryApiCompositionImpl<>(bases);
		...
		apiComposition
			...
			.innerJoin(Role.class, l -> roles)
				.on(e -> new Key(e.roleId), r -> new Key(r.roleId))
				.mapToEntity((re, r) -> {
					re.roleName = r.roleName;
				})
		.and()
			.innerJoin(Group.class, l -> groups)
				.on(e -> new Key(e.groupId), r -> new Key(r.groupId))
				.mapToEntity((re, r) -> {
					re.groupName = r.groupName;
				});

		List<ResultEntity> result = apiComposition.execute();
